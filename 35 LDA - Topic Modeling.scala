// Databricks notebook source exported at Wed, 27 Jul 2016 15:19:03 UTC
...
// MAGIC     Optimizers:
// MAGIC         EMLDAOptimizer using Expectation Maximization
// MAGIC         OnlineLDAOptimizer using Iterative Mini-Batch Sampling for Online Variational Bayes
...
// MAGIC %md To use the convenient Feature extraction and transformation APIs, we will convert our RDD into a DataFrame.
// MAGIC 
...
val corpusDF = corpus_body.zipWithIndex.toDF("corpus", "id")
...
val countVectors = vectorizer.transform(filteredDF).select("id", "features")
...
// MAGIC %md We will set the parameters needed to build our LDA model. We can also setMiniBatchFraction for the OnlineLDAOptimizer, which sets the fraction of corpus sampled and used at each iteration. In this example, we will set this to 0.8.
...
// MAGIC %md Note that using the OnlineLDAOptimizer returns us a LocalLDAModel, which stores the inferred topics of your corpus.
...
val newFilteredDF = remover.transform(tokenizedDF)
...
// Create new DataFrame of countVectors

val newCountVectors = vectorizer.transform(newFilteredDF).select("id", "features")

// COMMAND ----------

// Convert DF to RDD

val newLDACountVector = newCountVectors.map { case Row(id: Long, countVector: Vector) => (id, countVector) }
...
val new_ldaModel = new_lda.run(newLDACountVector)
...
val em_ldaModel = em_lda.run(newLDACountVector)

// COMMAND ----------

// MAGIC %md Note that the EMLDAOptimizer produces a DistributedLDAModel, which stores not only the inferred topics but also the full training corpus and topic distributions for each document in the training corpus.
...
// MAGIC %md To improve our results further, we could employ some of the below methods:
// MAGIC 
